...
function expandHelper(remaining, offset, ctx, flags) {
	...
	// A newsgroup is written by name; "news:" only ever belonged in link URLs.
...
// A token string as the reader sees it, for punctuation checks.
function plainText(value, entities) {
	return String(value)
		.replace(/\{@([a-z0-9-]+)\|([^|}]*)[^}]*\}/gu, '$2')
		.replace(/\{@([a-z0-9-]+)\}/gu, (whole, slug) => entities[slug]?.name || slug);
}
...
// A book's title is italic instead of quoted, followed by "Publisher, Year."
// An untitled item (an email, a notice) has a plain "description" instead.
function renderSource(key, source, ctx) {
	...
	if (source.author) {
		...
		text += /[.!?]$/u.test(plainText(author, ctx.entities)) ? `${author} ` : `${author}. `;
	}
	...
	if (source.description) {
		...
		text += `<em>${title}</em>.`;
		const imprint = [source.publisher && sourceParty(source.publisher, ctx.entities), source.date && `{y:${source.date}}`].filter(Boolean);
		if (imprint.length) text += ` ${imprint.join(', ')}.`;
	} else {
		text += `"${title}".`;
	}
...
function numberSources(source, ctx) {
	...
	for (let index = 0; index < order.length; index += 1) {
		...
		for (const field of [entry.author, entry.title, entry.publisher, entry.description, sourceNote(entry, ctx.name), entry.archive?.label]) {
...
function generalSortKey(ctx, key) {
	...
	return `${authorName} ${entry.title || entry.description || ''}`.toLowerCase().replace(/[^a-z0-9 ]/gu, '').trim();
}
...
function renderEntitiesData(loaded) {
	...
	for (const [slug, entry] of Object.entries(entities)) {
		...
		// A person's card lists the sites run under their name or any of their handles.
		if (entry.kind === 'person') {
			const owners = new Set([slug, ...handles]);
			const sites = Object.keys(entities).filter(other => owners.has(entities[other].owner));
...
function renderSnippets(loaded) {
	...
	for (const [key, source] of Object.entries(registry.loadSources(registry.sourcesPath, { local: false }))) {
		snippets[`[@${key}`] = { scope, prefix: `[@${key}`, body: `[@${key}]`, description: String(source.title || source.description || key).replace(/\{[^}]*\}/gu, '').slice(0, 80) };
	}
