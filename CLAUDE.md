...
- **Ephemeral preview layers** (`layer.isPreview`, Auto Glitter session) are excluded from history snapshots, the layers list, export, and project save; undo/redo is disabled while a session is active. Any new code that enumerates or serializes `editor.layers` must respect the flag. Their paint uses `GlitterManager.markPaintTransient` (no paintHistory snapshot) until commit.
