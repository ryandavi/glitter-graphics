...
- **Adding a layer type or tool:** follow `docs/!old/LAYER-TYPE-CONTRACT.md` — one manager class implementing the shared interface, one `LAYER_UI_CONFIG` entry, one GifExporter export-plan entry, panel markup + guide mirror. If it needs more than that, fix the dispatch site, don't add a branch.
- **Cross-manager logic goes in plain root scripts** (`utils.js`, `color-adjust.js`, `effect-source.js`, `slot-effects.js`) with thin delegating methods kept on the classes — never inheritance between layer managers.
- **Config:** any user-tunable or twice-used value belongs in `js/config.js` under the existing tree. Inline `??` fallbacks that restate CONFIG defaults are forbidden.
