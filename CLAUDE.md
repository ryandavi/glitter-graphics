...
- Don't run full Playwright verification unless asked — Ryan does manual testing himself. Quick suites that ARE expected: `node tests/touch-smoke.js` and `node tests/touch-handle-verify.js` (invoke with `node`, not npm).
...
- **Never use `ctx.filter`** — unsupported on Safari/iOS and iOS export is a supported path. Pixel-level math instead (see js/color-adjust.js for the pattern).
...
- **Masks are binarized** (`CONFIG.textLayers.crispEdges`, default true): partial-alpha mask edges fringe against the GIF transparency key (magenta) on transparent exports. Any new mask source (text, shapes) must go through the same threshold step.
...
- Config/state lives in `js/config.js` (`CONFIG`, `LayerType`, `ToolType`, `LAYER_UI_CONFIG`). New layer types register in `LAYER_UI_CONFIG` (desktop sections + `mobileSettingsSections`) — that is what makes mobile drawers work.
...
- Debug output goes through `dbg()` (js/debug.js), never bare `console.log`.
...
Active plan: `docs/TOOL-EXPANSION-PLAN.md` (brush/eraser split, shift straight lines, Wacom eraser fix, glitter HSV color adjust, shape tool) — read it before touching MaskEditor, TextGlitterManager, GlitterManager, or GifExporter. Earlier feature plans and their decisions live in `docs/` (UX-PLAN-2, TEXT-LAYOUT-SPEC, TOUCH-PLAN, etc.); they record *why* things are the way they are.
