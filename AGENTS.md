...
| Writing or editing any modal page (`content/src`), its entities or its sources | [content/AUTHORING.md](content/AUTHORING.md): the closed token list, commands, and the rules for slugs and sources |
| Editing the article pages' prose | `docs/local/HISTORY-PAGE-STYLE-GUIDE.md` (local-only) |
...
- **Verification:** don't run full Playwright verification unless asked; Ryan does manual testing. Always run `node tests/ui/touch-smoke.js` and `node tests/ui/touch-handle-verify.js` (with `node`, not `npm`). When touching export, effect sources, or the text or shape managers, also run `node tests/parity/export-parity.js` and `node tests/parity/shape-border-verify.js`. `npm run lint` must stay clean. Details: [tests/README.md](tests/README.md).
...
- **Export fragility test** after touching `SceneCompositor`, an exporter, or frame handling: add an animated sticker, export, edit, undo, then export again. Also export twice in a row. Outputs must be stable (byte-identical when nothing changed).
...
- **Where values live:** static defaults and tunables go in the frozen `CONFIG` (`js/core/config.js`), which is never assigned at runtime. Any user-tunable or twice-used value belongs there, and inline `??` fallbacks that restate a CONFIG default are forbidden. Runtime user preferences go in `PREFERENCES` (`js/core/preferences.js`), and export settings in `EXPORT_SETTINGS_SCHEMA` (`js/ui/settings-store.js`). Layer types live in `LayerType`/`LAYER_UI_CONFIG` (`js/core/layer-types.js`), sidebar structure in `PANEL_SCHEMAS` (`js/ui/panel-schemas.js`), release history in `RELEASES` (`js/core/releases.js`), tools in `TOOLS` (`js/core/tools.js`: one entry per tool, toolbar buttons render from it), commands and keyboard shortcuts in `COMMANDS` (`js/core/commands.js`), settings-modal rows in `EXPORT_SETTINGS_LAYOUT` / `APP_SETTINGS_LAYOUT` (`js/ui/settings-store.js`), and memory budgets in `CONFIG.memory`.
- **Layer types:** a new type registers itself with `registerLayerType` in its own `js/layers/types/<type>.js` (desktop sections plus `mobileSettingsSections`, which is what makes mobile drawers work, and its paint slots) and follows [docs/LAYER-TYPE-CONTRACT.md](docs/LAYER-TYPE-CONTRACT.md). If it needs more than the contract lists, fix the dispatch site; don't add a type branch.
...
- **Generated content:** every modal page in `modals/` is generated from `content/src/*.src.html`, with names from `content/entities.json` and citations from `content/sources.json`. Edit the source and run `node tools/build-modals.js`; never edit the generated modal files, `css/generated/`, `js/generated/` or `.vscode/content.code-snippets` by hand. Follow [content/AUTHORING.md](content/AUTHORING.md): never guess a slug or invent a source, and finish with `node tools/build-modals.js --check` at 0 errors. The `data/*.json` asset manifests are generated by the admin export; see [admin/README.md](admin/README.md).
- **UI:** sidebar panels come from `PANEL_SCHEMAS` through `js/ui/panel-renderer.js`, never from copied markup. The guide's tool headings, panel titles and shortcut list are generated from `TOOLS`, `PANEL_SCHEMAS` and `COMMANDS` (`{ui-tool:…}`, `{ui-panel:…}`, `{ui-shortcuts}` in `content/src/guide.src.html`); rebuild it after changing any of them, and write new prose in the source. Icons, buttons, panel naming and SCSS rules are in [docs/UI-CONVENTIONS.md](docs/UI-CONVENTIONS.md).
...
- **Cross-manager logic** goes in the responsibility folders (`js/core`, `js/effects`, `js/paint`, `js/transforms`, `js/systems`, `js/ui`), with thin delegating methods kept on the classes. Never use inheritance between layer managers. Keep only the application entrypoint at `js/app.js`; workers belong in `js/workers`, and anything a worker `importScripts` must be DOM-free.

## `js/` layout and naming

The current layout:

- `js/layers`, `js/assets`, `js/export`, `js/systems`: stateful classes grouped by responsibility. **The filename is the class it exports** (`GlitterManager.js` → `class GlitterManager`), in PascalCase.
- `js/core`, `js/effects`, `js/paint`, `js/transforms`, `js/ui`: responsibility folders for shared logic, with **kebab-case** filenames and no ALL-CAPS. `effects/` is worker-safe pixel math; `paint/` owns paint-source, slot, mask, and compositor logic.
...
`js/` is organized by domain. A move changes script paths in `index.html`, workers, test harnesses and tools; run `node tools/bump-cache.js` afterward.
