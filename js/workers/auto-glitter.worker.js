...
self.onmessage = ({ data }) => {
	...
	try {
		if (data.type === 'segment') {
			segmentCache = segmentImage(new Uint8ClampedArray(data.pixels), data.width, data.height, data.options);
		...
		if (data.type === 'reduce') {
			...
			const result = reduceSegment(segmentCache, data.colorCount, data.options, data.swatches);
...
function reduceSegment(cache, requestedCount, options, swatches) {
...
function assignSuggestedSwatches(palette, swatches, options) {
	...
	for (const entry of palette) {
		...
		for (let index = 0; index < preparedSwatches.length; index++) {
			...
			const closestColor = colors.reduce((closest, color) => distanceSquared(entry.lab, color) < distanceSquared(entry.lab, closest) ? color : closest, colors[0]);
			const distance = distanceSquared(entry.lab, preparedSwatches[index].primary) * options.swatchPrimaryWeight + distanceSquared(entry.lab, closestColor) * (1 - options.swatchPrimaryWeight);
