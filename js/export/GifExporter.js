...
class GifExporter {
	constructor(options = {}) {
		...
		this.canvas = createAppCanvas(0, 0, 'export/GifExporter');
		this.ctx = this.canvas.getContext('2d', { willReadFrequently: true });

		this.helperCanvas = createAppCanvas(0, 0, 'export/GifExporter');
		this.helperCtx = this.helperCanvas.getContext('2d');
		this.layerBlendCanvas = createAppCanvas(0, 0, 'export/GifExporter');
		this.layerBlendCtx = this.layerBlendCanvas.getContext('2d', { willReadFrequently: true });
		this.patternSourceCanvas = createAppCanvas(0, 0, 'export/GifExporter');
		...
		switch (layer?.type) {
			...
			case LayerType.STICKER: {
				...
				const scratch = {
					...
					shadowFillCanvas: createAppCanvas(0, 0, 'export/GifExporter')
				};
	...
	async _prepareExportContext(params) {
		...
		const watermarkCanvas = watermark ? createAppCanvas(0, 0, 'export/GifExporter') : null;
	...
	_createMaskCanvas(rawMaskData, width, height) {
		const c = createAppCanvas(0, 0, 'export/GifExporter');
		c.width = width;
		c.height = height;
