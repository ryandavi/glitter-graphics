...
/**
 * Renders the About modal's Timeline section from ABOUT_TIMELINE (see
 * js/ui/about-timeline-data.js) and wires up its Topic/Country/Decade/company
 * filters. modals-wiring.js calls this from aboutModal's onContentLoaded,
...
function initAboutTimeline(modalBody) {
	const filtersEl = modalBody.querySelector('#AboutTimelineFilters');
	const listEl = modalBody.querySelector('#AboutTimelineList');
	const summaryEl = modalBody.querySelector('#AboutTimelineSummary');
	...
	const topic = makeSelect('AboutTimelineTopic', 'Topic', topicOptions);
	const country = makeSelect('AboutTimelineCountry', 'Country', countryOptions);
	const decade = makeSelect('AboutTimelineDecade', 'Decade', decadeOptions);

	const entityWrap = document.createElement('div');
	entityWrap.className = 'timeline-filter timeline-filter-search';
	const entityLabel = document.createElement('label');
	entityLabel.htmlFor = 'AboutTimelineEntity';
	...
	entityInput.id = 'AboutTimelineEntity';
	entityInput.setAttribute('list', 'AboutTimelineEntityList');
	entityInput.setAttribute('autocomplete', 'off');
	entityInput.placeholder = 'e.g. Yahoo, Blingee, GeoCities…';
	const datalist = document.createElement('datalist');
	datalist.id = 'AboutTimelineEntityList';
